...
@section('content')
        ...
        <div class="gridUserCreateFirstRow">
            <div class="element-wrapper">
                <div class="element-box">
                   <form method="POST" action="/admin/booking/{{ $booking->id }}" enctype="multipart/form-data">
                        @method('PATCH')
                        @csrf()
                                ...
                                <select class="search-single custom-select" name="user_id" required>
                                    <optgroup label="User">
                                        @foreach ($users as $user)
                                            <option value="{{ $user['id'] }}" {{old('user_id',$booking->user_id) == $user->id ? 'selected' : ''}}>{{ $user['id'] }} ● {{ $user['first_name'] }} {{ $user['last_name'] }}</option>
                                        @endforeach
                                    </optgroup>
                                </select>
                            </div>
    ...
    @section('js_booking_edit_select')
        <script>

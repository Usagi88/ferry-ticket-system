...
@section('content')
            ...
            <div class="element-wrapper">
                <div class="element-box">
                   <form method="POST" action="/admin/role/{{$role->id}}">
                        @csrf
                        @method('PATCH')
                        <h5 class="form-header">Edit Role</h5>
                        <div class="form-desc">Edit roles & permissions</div>
                        <div class="form-group row">
                            <label class="col-form-label col-sm-4" for=""> Role Name</label>
                            <div class="col-sm-8"><input class="form-control" id="role_name" name="role_name" type="text" placeholder="Role Name" value="{{$role->name}}" required></div>
                        </div>
                        <div class="form-group row">
                            <label class="col-form-label col-sm-4" for=""> Role Slug</label>
                            <div class="col-sm-8"><input class="form-control" id="role_slug" name="role_slug" type="text" placeholder="Role Slug" value="{{$role->slug}}" required></div>
                        </div>
                        <div class="form-group row">
                            <label class="col-form-label col-sm-4" for="">Assign Permission</label>
                            <div class="col-sm-8"><input data-nametag="tags-manual-suggestions" name="permission" class="form-control" id="permission_roles" value="{{$role->permissions}}"></div>
                        </div>
    ...
    @section('css_role_edit_tags')
        <link rel="stylesheet" href="{{ asset('temp/vendor/css/admin/bootstrap-tagsinput.css') }}">
    @endsection



    @section('js_role_edit_permissionShow')
        <script>
            //This function is to get all permissions of role
            //Then we clean it up by removing spaces
            //After that we are adding it to the field
            var rolePermissions = {!! json_encode($role->permissions->toArray(), JSON_HEX_TAG) !!};
            ...
            for(let rolepermission in rolePermissions){//it will check each checkbox of that permission and add to input
                var changeString = rolePermissions[rolepermission].slug.trim();//slug
                var permName = rolePermissions[rolepermission].name.trim();//name
                
                $('#' + changeString).prop("checked", true);
                if($('#'+changeString).is(':checked')){
                    tagify.addTags(permName);

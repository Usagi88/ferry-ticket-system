...
@section('content')
        ...
        <div class="gridAssignVesselCreateFirstRow">
            <div class="element-wrapper">
                <div class="element-box">
                   <form method="POST" action="/admin/user/{{ $user->id }}/profile/my-assigned-vessel/{{$myAssign->id}}">
                        @method('PATCH')
                        @csrf()
                                    ...
                                    <optgroup label="Vessel">
                                        @if (!$vessels->isEmpty() && $vessels != null )
                                            @foreach ($vessels as $vessel)
                                                <option value="{{ $vessel['id'] }}" {{old('vessel_id',$myAssign->vessel_id) == $vessel->id ? 'selected' : ''}}>{{ $vessel['id'] }} ● {{ $vessel['name'] }}</option>
                                            @endforeach
                                        @endif
                                    </optgroup>
                                </select>
    ...
    @section('js_my_assign_vessel_edit_select')
        <script>

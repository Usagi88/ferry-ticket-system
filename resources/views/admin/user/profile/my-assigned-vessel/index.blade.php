    ...
    <div class="assignVesselIndexGridContainer">
            ...
            <div class="cardNew card-icon-bg card-icon-bg-primary o-hidden mb-4">
                <div class="card-body text-center">
                    ...
                    <div class="content">
                        ...
                        <p class="lead text-primary-black text-24 mb-2">{{ $user->assignedVessels->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        @can('create', App\Models\Vessel::class)
            <div class="tableCreateBtn">
                <a href="/admin/user/{{$user->id}}/profile/my-assigned-vessel/create" class="btnTest btn-primaryTest rippleTest" role="button" aria-pressed="true">Assign Vessel</a>
            </div>
        @endcan
                ...
                <table class="myTable">
                    ...
                    <tbody>
                        @foreach ($myAssigns as $myAssign)
                            @if (!$myAssign->assignedVessels->isEmpty() && $myAssign->assignedVessels != null )<!--If assigned vessels are not empty and null -->
                                @foreach ($myAssign->assignedVessels as $vessel)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="datatableActionButton">
                                                <a href="/admin/user/{{$user->id}}/profile/my-assigned-vessel/{{ $vessel->pivot['id'] }}" data-toggle="tooltip" data-placement="auto" title="Show"><i class="far fa-eye"></i></a>
                                                @can('create', App\Models\Vessel::class)
                                                    <a href="/admin/user/{{$user->id}}/profile/my-assigned-vessel/{{ $vessel->pivot['id'] }}/edit" data-toggle="tooltip" data-placement="auto" title="Edit"><i class="far fa-edit"></i></a>
                                                @endcan
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed){
                            $.ajax({
                                ...
                                url:"/admin/user/" + userID +"/profile/my-assigned-vessel/"+id,
                                data:{
                                    "_token": "{{ csrf_token() }}",
                                },
                                success: function(response) {
                                    Swal.fire(
                                        'Deleted!',
                                        'Your Assigned Vessel has been deleted.',
                                        'success'
                                    )
                                ...
                                failure: function (response) {
                                    Swal.fire(
                                        'Internal Error',
                                        'Your Assigned Vessel was not deleted.',
                                        'error'
                                    )
                                }
                            });
